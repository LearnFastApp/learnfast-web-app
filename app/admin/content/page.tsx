...
import {
  ...
  Plus, ShieldCheck, Trash2, Video, X,
} from "lucide-react";
...
export default function AdminContentPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    if (!res.ok) { setError("Failed to load content."); setLoading(false); return; }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (formType === "pdf" && formFile) {
        const path = `premium-content/${Date.now()}_${formFile.name}`;
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        <div className="flex items-start justify-between mb-8">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-xs font-semibold text-violet-400 uppercase tracking-widest">Platform Admin</p>
            </div>
        ...
        {showForm && (
          <div className="mb-8 bg-[#0f172a] border border-[#1e293b] rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-sm font-semibold text-white">Add premium content</h2>
        ...
        {items.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/10 p-10 text-center text-slate-500">
            No premium content yet — click <strong className="text-slate-300">Add content</strong> to upload your first resource.
          </div>
        ) : (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full text-amber-400 bg-amber-400/10">
                      Premium
                    </span>
                  </div>
                  {item.description && (
